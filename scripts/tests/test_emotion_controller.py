...
TEST_SOURCE = ROOT / "scripts" / "tests" / "emotion_controller_test.cc"
...
class EmotionControllerHostTest(unittest.TestCase):
    """LVGL-free host test for the Slice 6 EmotionController base-pose lookup."""

    def test_emotion_controller_compiles(self):
        """Always runs: compiles the Slice 6 sources with -Wall -Wextra -Werror.
        ...
        if compiler is None:
            self.skipTest("g++ or clang++ is required for the host emotion-controller test")
    ...
    def test_emotion_controller_compiles_and_runs(self):
        compiler = find_cxx_compiler()
        if compiler is None:
            self.skipTest("g++ or clang++ is required for the host emotion-controller test")
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            output = Path(directory) / "emotion_controller_test"
